...
- (UIImage*) getFrameAsImageAtIndex:(int)index
{
    ...
    if (frameData != nil)
    {
       ...
       // image =[self imageWithImage:image scaledToSize:CGSizeMake(274,73)];
    }
...
- (bool) GIFGetBytes: (int) length
{
    ...
    {
		GIF_buffer = [[[GIF_pointer subdataWithRange:NSMakeRange(dataPointer, length)] retain] mutableCopy];
        dataPointer += length;
		return YES;
	}
    else
